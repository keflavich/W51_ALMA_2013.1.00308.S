...
fig1.add_axes(ax)
data = (cutout_e2 * u.beam).to(u.K, beam_e2e8.jtok_equiv(masscalc.centerfreq)).value
norm = astropy.visualization.ImageNormalize(data, stretch=astropy.visualization.AsinhStretch())
...
fig1.add_axes(ax)
data = (cutout_e8*u.beam).to(u.K, beam_e2e8.jtok_equiv(masscalc.centerfreq)).value
norm = astropy.visualization.ImageNormalize(data, stretch=astropy.visualization.AsinhStretch())
im = ax.imshow(data, origin='lower', interpolation='none', cmap='gray', norm=norm)
ax.contour(struct.get_mask(), levels=[0.5], colors=['orange'], origin='lower')
ax.contour(trunk.get_mask(), levels=[0.5], colors=['red'], origin='lower')
...
fig1.add_axes(ax)
data = (cutout_north*u.beam).to(u.K, beam_north.jtok_equiv(masscalc.centerfreq)).value
norm = astropy.visualization.ImageNormalize(data, stretch=astropy.visualization.AsinhStretch())
im = ax.imshow(data, origin='lower', interpolation='none', cmap='gray', norm=norm)
#ax.contour(trunk.get_mask(), levels=[0.5], colors=['red'], origin='lower')
