...
diskycoords = pyregion.open(paths.rpath("e2e_disk_pvextract.reg"))[0].coord_list
diskycoords = coordinates.SkyCoord(["{0} {1}".format(diskycoords[jj], diskycoords[jj+1])
                                    for jj in (0,2,4)], unit=(u.deg, u.deg), frame='fk5')
diskypath = pvextractor.Path(diskycoords, 0.2*u.arcsec)
