...
\abstract{
    We present ALMA observations of a $\sim3\times1.5$ pc area in the W51
    ...
    modeling and brightness-temperature-based limits.  The observed region
    contains three high-mass YSOs that appear to be at the earliest stages of
    their formation, with no signs of ionizing radiation from their central
    sources.  The new data reveal  high gas and dust temperatures ($T > 100$ K)
    extending out to about 5000 AU from each of these sources, indicating that
    the forming MYSOs are able to heat a large volume of gas in which
    fragmentation is suppressed.  No distinct fragments are observed within
    this heated zone, though 75 sources representing either prestellar cores or
    some stage of protostars are detected in the observed field.  During the
    process of forming, high-mass stars heat a large volume (and
    correspondingly large mass) of gas, preventing it from fragmenting, and
    therefore keep a large reservoir available to feed from.  By contrast, more
    mature massive stars that illuminate compact \hii regions have little
    effect on their surrounding dense gas, hinting that these sources have
    completed most or all of their accretion.  
...
univeral) distribution,  massive stars O-stars (with $M>50 \msun$) almost
always form in a clustered fashion (in proto-clusters or proto-associations).
...
\Figure{figures/w51_overview_placeholder.png}
{An overview of the W51A region as seen by ALMA.  The grayscale shows
a 1.3mm ALMA image.  \todo{This is a placeholder}}
{fig:overview}{1}{10cm}
...
star with effective temperature $4\ee{4}$ K \citep{Pecault2013a}, however, see
...
{\todo{This figure is meant for discussion, not necessarily final publication.}
The radial mass profile exponent $\kappa_\rho$ as a function of radius.
The `fiducial value' used in the McKee/Tan model is $\kappa_\rho=-1.5$ (where
I've flipped the sign...), but importantly it is \emph{constant} at $t=0$.
What are the consequences of a non-constant $\kappa_\rho$?}
{fig:kapparho}{1}{12cm}
...
More importantly, fragments of such mass cannot be assembled in this core,
as they would be larger than the core at any given radius; the Jeans length
is comparable to the hot core size and suggests that only one total fragment
(the core itself) should exist.
%this is wrong
...
In (b), the dashed black line shows the $R_J=R$ line.  When $R_J > R$,
the gas is on average stable against Jeans collapse on the measured length
...
Our observations therefore support a monolithic core accretion model for the
formation of massive stars, except this monolithic core is not a simple initial
condition of the gas.  It is not clear how such cores are assembled in the first
place, but the key requirement for their formation is that a high-mass protostar
must form first.  
...
all without the presence of a central massive star.  Should these $>200$ \msun
cores have been present before high-mass star formation initiated, resting
at $T\sim20$ K as in a typical molecular cloud, they would have been subject
to Jeans fragmentation on a much smaller scale, and would have formed a cluster
of smaller stars.
...
the initial conditions were that initiated this formation.  Did the
...
object.  This toy model is analagous to the `cooperative accretion' mode
...
In this scenario, the highest mass stars would only be capable of forming
within dense, clustered environments.  The IMF would be built up by an inverse
...
would expect to see warm gas surrounding the \hchii regions.  Instead, we see
these stars barely interacting with the dense gas.  Perhaps these stars are
only effective at dense gas heating before they ignite Lyman continuum emission
and blow out cavities, and afterward they are merely uninteractive witnesses to
continued collapse.
...
or that the current high density of main-sequence stars originated from a
broader, lower-density distribution.  If the stars illuminating \hchii regions
formed somewhat earlier in a more distributed manner and fell into a common
central potential, they may have had little effect on the dense gas
temperature.

\section{Comparison to high-mass star formation theories}
\todo{REVISE THIS}
Two general classes of star formation theory, ``competitive accretion'' and
``turbulent core accretion'' have dominated the discussion of initial
conditions in high-mass star formation.  In their extreme forms, they can be
simplified to uniform seeds accreting different amounts of material
...
suppresses the formation of new small fragments, as was observed in simulations
with much weaker feedback by \citet{Bate2009a}. 
...
$\kappa_\rho=1.5$, as usually assumed \citep{McKee2003a,Zhang2011a}.  Figure
\ref{fig:kapparho} shows instead that the core is internally flatter and
externally steeper than this assumption.  Of course, the observations do not
correspond to the initial conditions, so this comparison could more fairly be
made with a later `snapshot' from those models.  Second, the more sophisticated
models of evolution from an initial turbulent core suggest that most of the
stellar (and accretion) radiation should escape via growing outflow cavities
\citep{Zhang2011a,Zhang2013h,Zhang2014e}.  The symmetry seen in the hot core
around e2e and its lack of correspondence with the outflowing material (Figure
...
\todo{this is mostly just spitballing; it probably ought to be removed:}
...
%\todo{Do I need a plot of gravitational pressure / thermal pressure vs radius?
%probably yes.}
