...
units of figure fraction.  The horizontal black lines show the detection
threshold of each of the transitions; points below these lines are ignored when
fitting, and instead the threshold itself is used.  The fitted temperature and
...
\citep[Section \ref{sec:ch3ohtem}; ][]{Goddi2016a}.  If the observed dust
...
hotter dust.  
Figure \ref{fig:hmradprof}b shows $M(R)$ using $T_{dust}=T_{\methanol}$, which
is a more reasonable guess at the true mass profile (though it is still a lower
limit; see \S \ref{sec:ch3ohtem}).
...
area are high enough, $L\gtrsim10^4\lsun$, to illuminate and excite molecularly
enhanced regions if they were surrounded by dense ($n(\hh)\gtrsim10^4$\percc)
gas.  Since no such regions are detected, we conclude that these \hii regions
are not surrounded by such high-density gas but instead are traveling through a
lower-density medium.  The counterexamples, e2w and d2, are extremely compact
and do exhibit some enhanced molecular emission around them, though not as much
as the cores, suggesting that they may be moving more rapidly through the dense
medium rather than being stationary with respect to it... \todo{explore this
idea a little more: what is the equilibration timescale of the $r>1000$ AU gas?
is it plausible that it just hasn't been heated?  If not, why don't d2/e2w light
the big regions?}
...
sharp edges likely reflect the radius at which where the temperature exceeds
...
gas at smaller radii, such as the nitrogenic species we detected, suggesting
that the cores are dominated by sublimated ices from $R\sim2000-5000$ AU
and by species formed in the gas phase at $R\lesssim2000$ AU.
...
$R_{\hii}<100 AU$, which is consistent with the presence of late-type massive
...
cooking  into one single serving the gas feast that would otherwise form a
small cluster.
...
There are also implications for `isolated' massive star formation.
...
similarly independent of the size of the parent cloud: high-mass stars will form
the same way whether in an `isolated' or `clustered' region because they govern
gas conditions in their own surroundings.
