...
\abstract{
    ...
    large mass) of gas, preventing it from fragmenting, and therefore keeping
    a large reservoir available to feed from.  (The observations provide a solution
...
univeral) distribution, very massive stars (VMS; $M>50 \msun$) almost always form
...
Models of high-mass star formation universally have difficulty getting enough
mass to a stellar radius to form very massive stars.  Generally, these models
...
accreted \citep{}, but geometric effects can circumvent this limit and allow
further accretion
\citep{Krumholz2005b,Krumholz2009a,Krumholz2009b,Kuiper2012a,Kuiper2013c,Rosen2016a}.
...
best cases \citep{Kuiper2015a,Kuiper2016a}.

Nature is clearly capable of producing such massive stars and some that are
even larger.  Within the LMC, stars up to $M\sim300$ \msun have been
spectroscopically identified \citep{Crowther2016a}, so we know it is possible
to form these.  Within our own Galaxy, UV and optical characterization of these
stars is not possible, but some infrared lines are indicators of such stars.
\citet{Barbosa2008a} identified an O3 and an O4 star ($M\gtrsim50$ \msun)
within the W51 IRS2 region, demonstrating that this region at least was in the
past capable of forming such stars.
...
\citep{Zhang1997a,Eisner2002a,Zapata2009a,Zapata2010a,Shi2010b,Shi2010a}.  The
...
environment where massive star feedback is already evident, but high-mass star
...
to simulations.
\citet{Krumholz2006a} suggest that accretion heating during the formation
of high-mass stars can heat their cores to $\gtrsim100$ K and therefore
suppress fragmentation into smaller stars as would be expected for cold cores.
While simulations have verified the conclusion that early-stage accretion
heating is important for low-mass star formation
\citep{Krumholz2007c,Offner2011b, Bate2012a,Bate2014b}, there have been
neither theoretical nor observational tests of this model for high-mass
stars. 
...
\ref{sec:corespatialdistribution}), photometry (\S \ref{sec:photometry}), and
statistical distributions (\S \ref{sec:distributionfunctions}).  We then
examine the source temperatures (\S \ref{sec:temperatures}) in order to
...
spatial scales.
Section \ref{sec:chemistrymaps} describes the observed (\S
\ref{sec:chemistrymapsobs}) chemical distribution and
...
from \hii regions.
Section \ref{sec:outflows} and its subsections describe outflows \todo{should
...
\todo{accretion and outflows (\S \ref{sec:accretionandoutflows})},
...
The corresponding masses are shown in Figure \ref{fig:masshistograms}
assuming the dust temperature is equal to the source's peak line
brightness temperature (Section \ref{sec:temperature}).
...
Tables \ref{tab:linesspw0}-\ref{tab:linesspw3}, rejecting those with poor fits.
The line brightnesses reported in the figure are the sum of the
continuum-subtracted peak line brightness and the continuum brightness.
Excepting CO and \formaldehyde, which are excluded from the plot, \methanol is
the brightest line toward most sources. 
...
\todo{This section may be eliminatable:}
...
forming stars.  The following conclusions....
