...
Sections \ref{sec:sftheories} and \ref{sec:cooperative} discuss implications
...
because it appears only in their 25 \um images and is invisible at shorted
...
comparable to North but presumably less luminous.
...
\Figure{figures/methanol_velocity_moments_e2wide.png}
...
\ref{sec:temperatures}, giving $c_s\sim0.5$ \kms.  With velocity dispersions
...
velocities from \citet{Goddia2016a} are adopted, the offset noted by
...
outflows (Section \ref{sec:outflows}) hints at the presence of accretion disks,
but measurement of a Keplerian rotation curve is necessary to determine their
presence definitively.

The characteristic signature of a Keplerian disk is a velocity profile that
rises from slow in the outskirts, almost certainly smaller than the turbulent
...
region, which renders any attempt at determining the width
of a given line nearly impossible.
...
center.  The line-to-continuum ratio also drops, which could be an indication
that the dust is becoming optically thick, preventing us from detecting the
high-velocity gas.  Indeed, an optically thick inner disk is theoretically
...
that the disks are either too small ($r<1000$ AU) or too optically thick to
...
\begin{equation}
EM = -3.05\ee{6} 
\left(\frac{T_e}{10^4\mathrm{K}}\right)^{-1}
...
The Kelvin-Helmholtz timescale for a massive star is extremely short,
$\tau_{KH} < 5\ee{4}$ yr for a $M>20$ \msun star, which suggests that these
stars should reach the main sequence and beging ionizing their surroundings
while they are still accreting \citep{Zinnecker2007a}.
...
are contracting more slowly than the Kelvin-Helmholtz timescale.  The very
large
mass reservoir suggests that high accretion rates are possible, and the bright
molecular outflows show that accretion is proceeding vigorously (though we have
not quantified the rate).  Rapid accretion, and in particular rapid and
\emph{variable} accretion, can change the properties of the underlying star,
bloating the star and reducing its effective photospheric temperature
\citep{Hosokawa2009a, Smith2012h, Hosokawa2016a}.  Such stars can achieve radii
$R\gtrsim200 \rsun \sim 1$ AU while retaining photospheric temperatures
$T\lesssim5000$ K.  Bloated central stars would therefore explain why they do
not produce \hii regions.
...
than the ionization rate, such that there is always fresh neutral material to
ionize at the surface of the star.  The critical rate for \hii region quenching
...
outflows in e2e and north suggests that, if there are multiple systems, they
are accreting from a common disk, though the changing direction of the outflows
