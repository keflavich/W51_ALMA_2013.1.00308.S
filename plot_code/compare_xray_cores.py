...
coredots, = ax.plot(cores.ra, cores.dec, 'b.', transform=tr_fk5, markersize=2,
                    zorder=50)
cmdots, = ax.plot(cmcoords.ra, cmcoords.dec, 'go', transform=tr_fk5, alpha=0.5,
                  zorder=1, markeredgecolor='none',)

dots, = ax.plot(w51moxc['RAJ2000'], w51moxc['DEJ2000'], 'r.', transform=tr_fk5,
                markersize=2, zorder=60)
#ax.axis(lims)
ax.axis([x1,x2,y1,y2])
...
fig.savefig(paths.fpath("moxc_points_on_cband_withcores.png"))
...
#ax.axis(lims)
ax.axis([x1,x2,y1,y2])
fig.savefig(paths.fpath("moxc_points_contours_on_cband_withcores.png"))
dots.set_visible(False)
fig.savefig(paths.fpath("moxc_contours_on_cband_withcores.png"))
pl.draw()
pl.show()
