#if not os.path.exists('w51_test_small.ms'):
#    assert split(vis='w51_spw3_continuum_flagged.ms',
...
clean(vis='w51_test_small.ms', imagename="test_mfs_dirty", field="", spw='',
      ...
      interactive=False, niter=0, threshold='50.0mJy', imsize=[512,512],
...
flagmanager(vis='w51_test_small_selfcal_2.ms', mode='save',
            versionname='backup')
...
split(vis="w51_test_small_selfcal_2.ms",
      outputvis="w51_test_small_selfcal_3.ms",
      datacolumn="corrected")

os.system('rm -rf test_selfcal_3_mfs.*')
clean(vis='w51_test_small_selfcal_3.ms',
      imagename="test_selfcal_3_mfs",
      field="", spw='', mode='mfs', outframe='LSRK',
      interpolation='linear', imagermode='mosaic', interactive=False,
      niter=1000, threshold='50.0mJy', imsize=[512,512], cell='0.06arcsec',
      phasecenter='J2000 19h23m43.905 +14d30m28.08', weighting='briggs',
      usescratch=True, pbcor=False, robust=-2.0)
exportfits('test_selfcal_3_mfs.image',
           'test_selfcal_3_mfs.image.fits', dropdeg=True,
           overwrite=True)
...
print("selfcal3: peak={1:0.5f} sigma={0:0.5f}".format(fits.getdata('test_selfcal_3_mfs.image.fits')[:200,:200].std(), fits.getdata('test_selfcal_3_mfs.image.fits').max()))
